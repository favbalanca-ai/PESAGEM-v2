...
function salvarPesagem(o){
  ...
  var st=s(o.statusPeso).replace(/(Finalizado\s*[-—]\s*)+/gi,"").trim();if(!/CONFORME|ATEN|EXCEDIDO/i.test(st))st="";
...
function lerDocumento(frame, tipo){
  ...
  if(String(tipo||"").toUpperCase()==="CNH"){
    ...
    prompt="Analise esta CNH brasileira (Carteira Nacional de Habilitacao) e extraia SOMENTE dois dados do titular.\nRetorne APENAS JSON puro:\n{\"motorista\": \"NOME COMPLETO\", \"cpf\": \"000.000.000-00\", \"confianca\": \"alta\", \"obs\": \"\"}\n\nREGRAS IMPORTANTES:\n- motorista = valor do campo \"NOME E SOBRENOME\" (o titular da CNH), em MAIUSCULAS. NUNCA use os nomes do campo \"FILIACAO\" (pai/mae). Leia o nome EXATAMENTE como esta escrito, letra por letra — NAO substitua um nome incomum por um nome comum e NAO invente/adivinhe. Confira tambem contra a linha MRZ no rodape (ex.: 'RONIE<<ALVES<DE<PAIVA' = RONIE ALVES DE PAIVA). Se o nome estiver ilegivel ou borrado, retorne null em vez de chutar.\n- cpf = campo \"CPF\", formato 000.000.000-00 (11 digitos). NAO confundir com \"Nº REGISTRO\" (11 digitos, geralmente em vermelho) nem com \"DOC. IDENTIDADE / RG\".\n- Leia o CPF DIGITO A DIGITO com muita atencao, sem trocar numeros parecidos (9 com 0, 1 com 7, 5 com 6, 3 com 8). O CPF possui DIGITOS VERIFICADORES: os 2 ultimos numeros validam os 9 primeiros pelo algoritmo padrao de CPF. Antes de responder, confira mentalmente se o CPF lido e valido; se nao fechar, releia os digitos com cuidado ate obter um CPF valido.\n- Nao extraia nenhum outro campo. Retorne null para o que nao encontrar. Nao invente dados.";
  }else{
